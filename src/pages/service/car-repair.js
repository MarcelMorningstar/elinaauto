...
export default function carRepair({ repair, index }) {
  return (
    <Layout heading={index.heading} titles={[index.title1, index.title2, index.title3, index.title4]}>
      <List tag="car-repair" data={repair.pageBuilder} />
    </Layout>
  )
}

export async function getStaticProps({ locale }) {
  const repair = await client.fetch(`*[_type == "page" && tag == "car-repair" && language == "${ locale }"][0]{
